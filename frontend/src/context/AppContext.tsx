...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Initial live synchronization with backend
  useEffect(() => {
    async function loadBackendData() {
      try {
        const [prodRes, batchRes, custRes, supRes, saleRes, purRes, colRes, auditRes] = await Promise.allSettled([
          api.getProducts(),
          api.getBatches(),
          api.getCustomers(),
          api.getSuppliers(),
        ...
        if (prodRes.status === 'fulfilled' && prodRes.value.data && prodRes.value.data.length > 0) setProducts(prodRes.value.data);
        if (batchRes.status === 'fulfilled' && batchRes.value.data && batchRes.value.data.length > 0) setBatches(batchRes.value.data);
        if (custRes.status === 'fulfilled' && custRes.value.data && custRes.value.data.length > 0) setCustomers(custRes.value.data);
        if (supRes.status === 'fulfilled' && supRes.value.data && supRes.value.data.length > 0) setSuppliers(supRes.value.data);
        if (saleRes.status === 'fulfilled' && saleRes.value.data && saleRes.value.data.length > 0) setSales(saleRes.value.data);
        if (purRes.status === 'fulfilled' && purRes.value.data && purRes.value.data.length > 0) setPurchases(purRes.value.data);
        if (colRes.status === 'fulfilled' && colRes.value.data && colRes.value.data.length > 0) setCollections(colRes.value.data);
        if (auditRes.status === 'fulfilled' && auditRes.value.data && auditRes.value.data.length > 0) setAuditLogs(auditRes.value.data);
      } catch {
        // Fallback gracefully to offline cache/mock dataset
      }
    }
    loadBackendData();
  }, []);
  ...
  const addProduct = (prodData: Omit<Product, 'id'>) => {
    ...
    api.createProduct(prodData).catch(() => {});
  ...
  const addSupplier = (supplierData: Omit<Supplier, 'id' | 'code'>) => {
    ...
    api.createSupplier(supplierData).catch(() => {});
  ...
  const recordStockAdjustment = (productId: string, batchNumber: string, quantityDelta: number, reason: string, isDamage: boolean = false) => {
    ...
    api.adjustStock({ productId, batchNumber, quantityDelta, reason, isDamage }).catch(() => {});
  ...
  const writeOffBatch = (batchId: string) => {
    ...
    api.writeOffBatch(batchId).catch(() => {});
  ...
  const createPurchase = (purchaseData: Omit<PurchaseTransaction, 'id' | 'invoiceNumber'>) => {
    ...
    api.createPurchase(newPurchase).catch(() => {});
  ...
  const recordCollectionPayment = (customerId: string, amount: number, method: 'Cash' | 'Cheque' | 'UPI' | 'NEFT/RTGS', notes?: string) => {
    ...
    api.recordPayment({ customerId, amount, paymentMethod: method, notes }).catch(() => {});
